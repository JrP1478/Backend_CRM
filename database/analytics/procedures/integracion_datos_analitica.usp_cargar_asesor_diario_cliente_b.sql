/*
===============================================================================
CLIENTE_B ADVISOR DAILY - DEFINICIÓN CANÓNICA VERSIONADA
Procedimiento: integracion_datos_analitica.usp_cargar_asesor_diario_cliente_b

Regla de atribución de pagos CLIENTE_B:
...
/* ============================================================================    CLIENTE_B - REANUDAR BACKFILL ADVISOR DESDE 2026-08     Corrige el conflicto:      UX_puente_supervisor_asesor_clave_asesor_vigente_desde      duplicate key (110, 2026-09-01)     Causa:      durante el backfill histórico se intentaba crear una relación CURRENT      desde el día siguiente a cada cierre mensual. Agosto intentaba crear      2026-09-01, fecha que ya existe como observación histórica de septiembre.     Solución:      - histórico: @CrearJerarquiaActual = 0      - último corte operativo: @CrearJerarquiaActual = 1     NO reprocesa enero 2024 - julio 2026.    ============================================================================ */  CREATE OR ALTER PROCEDURE     integracion_datos_analitica.usp_cargar_asesor_diario_cliente_b (     @FechaCorte DATE,     @Aplicar BIT = 1,     @MostrarResultado BIT = 0,     @CrearJerarquiaActual BIT = 1 ) AS BEGIN     SET NOCOUNT ON;     SET XACT_ABORT ON;      DECLARE         @IdCliente INT = 59,         @IdContrato INT = 246,         @IdGrupo INT = 194,         @ClaveCliente INT,         @ClaveCampana INT,         @FechaInicio DATE,         @FechaHasta DATETIME2(3),         @AhoraUtc DATETIME2(3) = SYSUTCDATETIME(),         @FechaHoraUltimoOrigen DATETIME2(3),         @IdUltimoOrigen BIGINT;      IF @FechaCorte IS NULL         THROW 52600, 'Debe indicar @FechaCorte.', 1;      IF @FechaCorte > CONVERT(DATE, GETDATE())         THROW 52601, 'La fecha de corte no puede estar en el futuro.', 1;      SET @FechaInicio = DATEFROMPARTS(YEAR(@FechaCorte), MONTH(@FechaCorte), 1);     SET @FechaHasta = DATEADD(DAY, 1, CONVERT(DATETIME2(3), @FechaCorte));      SELECT @ClaveCliente = clave_cliente     FROM analitica.dim_cliente     WHERE id_cliente_crm = @IdCliente       AND es_activo = 1;      IF @ClaveCliente IS NULL         THROW 52602, 'CLIENTE_B no existe o no está activo en analitica.dim_cliente.', 1;      SELECT @ClaveCampana = clave_campana     FROM analitica.dim_campana     WHERE clave_cliente = @ClaveCliente       AND anio_campana = YEAR(@FechaCorte)       AND mes_campana = MONTH(@FechaCorte);      IF @ClaveCampana IS NULL         THROW 52603, 'No existe la campaña CLIENTE_B del corte solicitado.', 1;      EXEC integracion_datos_analitica.usp_asegurar_rango_fechas         @FechaInicio,         @FechaCorte;       /* ------------------------------------------------------------------------        Carteras físicas productivas del mes.        ------------------------------------------------------------------------ */     IF OBJECT_ID('tempdb..#Carteras') IS NOT NULL DROP TABLE #Carteras;      SELECT         ca.nId_Cartera,         dc.clave_cartera     INTO #Carteras     FROM dbo.Crm_Cartera ca     INNER JOIN analitica.dim_cartera dc         ON dc.clave_cliente = @ClaveCliente        AND dc.id_cartera_origen = ca.nId_Cartera     WHERE ca.nId_Cliente = @IdCliente       AND ca.nId_Contrato = @IdContrato       AND ca.nId_Grupo = @IdGrupo       AND ca.nAnioCar = YEAR(@FechaCorte)       AND ca.nCampCar = MONTH(@FechaCorte)       AND ca.cCiclo IN ('CICLO 04','CICLO 11','CICLO 18','CICLO 25')       AND UPPER(ISNULL(ca.cCar_Nombre,'')) NOT LIKE '%BORRADOR%';      IF NOT EXISTS (SELECT 1 FROM #Carteras)         THROW 52604, 'No existen carteras CLIENTE_B productivas para la campaña.', 1;       /* ------------------------------------------------------------------------        Deudores elegibles al corte. Mismo universo documental del ETL CLIENTE_B.        ------------------------------------------------------------------------ */     IF OBJECT_ID('tempdb..#BaseDeudor') IS NOT NULL DROP TABLE #BaseDeudor;      SELECT DISTINCT         c.nId_Cartera,         c.clave_cartera,         dc.nId_PersDeudor     INTO #BaseDeudor     FROM #Carteras c     INNER JOIN dbo.Crm_DocxCobrar dc         ON dc.nId_Cliente = @IdCliente        AND dc.nId_Cartera = c.nId_Cartera     INNER JOIN dbo.Crm_DocxCobrarParam dp         ON dp.nId_Cliente = dc.nId_Cliente        AND dp.nId_Cartera = dc.nId_Cartera        AND dp.nId_DocxCobrar = dc.nId_DocxCobrar     WHERE ISNULL(dc.nId_DocxCobrarEst,1) <> 8       AND UPPER(LTRIM(RTRIM(ISNULL(dc.cCampo4,'')))) <> 'RETIRADO'       AND dc.dDoc_FecIngreso < @FechaHasta;       /* ------------------------------------------------------------------------        Fuente humana MTD.        ------------------------------------------------------------------------ */     IF OBJECT_ID('tempdb..#Source') IS NOT NULL DROP TABLE #Source;      SELECT         f.clave_fecha,         c.clave_cartera,         o.nId_Cartera,         CONVERT(INT, o.nId_UsuOpe) AS id_asesor_origen,         CONVERT(BIGINT, o.nId_DocxCobrarOpe) AS id_operacion_origen,         CONVERT(BIGINT, o.nId_PersDeudor) AS id_deudor_origen,         COALESCE(o.dDoc_FecIngresoGes, o.dDocCobOpe_FecIni)             AS fecha_hora_gestion,          NULLIF(LTRIM(RTRIM(u.cUsr_NroDoc)),'')             AS documento_asesor,          NULLIF         (             LTRIM(RTRIM(CONCAT(                 ISNULL(u.cUsr_Nombres,''),                 CASE WHEN NULLIF(LTRIM(RTRIM(u.cUsr_ApePat)),'') IS NOT NULL THEN ' ' ELSE '' END,                 ISNULL(u.cUsr_ApePat,''),                 CASE WHEN NULLIF(LTRIM(RTRIM(u.cUsr_ApeMat)),'') IS NOT NULL THEN ' ' ELSE '' END,                 ISNULL(u.cUsr_ApeMat,'')             ))),             ''         ) AS nombre_asesor,          CONVERT(BIT, ISNULL(u.bEstado,0)) AS asesor_es_activo,          COALESCE         (             NULLIF(o.nId_UsrLider,0),             NULLIF(u.nid_UsuSuper,0)         ) AS id_supervisor_origen,          UPPER         (             COALESCE             (                 NULLIF(LTRIM(RTRIM(cod.cParam04)),''),                 NULLIF(LTRIM(RTRIM(tc.indicador_equiv)),''),                 ''             )         ) AS codigo_contacto      INTO #Source      FROM dbo.Crm_DocxCobrarOpe o      INNER JOIN #BaseDeudor b         ON b.nId_Cartera = o.nId_Cartera        AND b.nId_PersDeudor = o.nId_PersDeudor      INNER JOIN #Carteras c         ON c.nId_Cartera = o.nId_Cartera      INNER JOIN dbo.Crm_Usuario u         ON u.nId_Usuario = o.nId_UsuOpe        AND u.nId_PerfilGest IS NULL      INNER JOIN dbo.Crm_OpeCodCliOut cod         ON cod.nId_Cliente = o.nId_Cliente        AND cod.nId_OpeCodCliOut = o.nId_OpeCodOut      LEFT JOIN dbo.Crm_TipoContacto tc         ON tc.nId_TipoContacto = cod.nId_TipoContacto      INNER JOIN analitica.dim_fecha f         ON f.fecha_calendario =             CONVERT(DATE, COALESCE(o.dDoc_FecIngresoGes, o.dDocCobOpe_FecIni))      WHERE o.nId_Cliente = @IdCliente       AND o.tip_gestion IN (1,2,4)       AND o.nId_UsuOpe IS NOT NULL       AND COALESCE(o.dDoc_FecIngresoGes, o.dDocCobOpe_FecIni) >= @FechaInicio       AND COALESCE(o.dDoc_FecIngresoGes, o.dDocCobOpe_FecIni) < @FechaHasta;      IF NOT EXISTS (SELECT 1 FROM #Source)         THROW 52605, 'No existen gestiones humanas CLIENTE_B para el corte solicitado.', 1;      SELECT         @FechaHoraUltimoOrigen = MAX(fecha_hora_gestion),         @IdUltimoOrigen = MAX(id_operacion_origen)     FROM #Source;       /* ------------------------------------------------------------------------        Dimensión asesor.        ------------------------------------------------------------------------ */     IF OBJECT_ID('tempdb..#Asesores') IS NOT NULL DROP TABLE #Asesores;      ;WITH Ranked AS     (         SELECT             id_asesor_origen,             documento_asesor,             nombre_asesor,             asesor_es_activo,             fecha_hora_gestion,             id_operacion_origen,             ROW_NUMBER() OVER             (                 PARTITION BY id_asesor_origen                 ORDER BY fecha_hora_gestion DESC, id_operacion_origen DESC             ) AS rn         FROM #Source     )     SELECT         id_asesor_origen,         documento_asesor,         COALESCE(nombre_asesor, CONCAT('Usuario ', id_asesor_origen))             AS nombre_asesor,         asesor_es_activo     INTO #Asesores     FROM Ranked     WHERE rn = 1;      UPDATE a     SET         a.documento_asesor = s.documento_asesor,         a.nombre_asesor = s.nombre_asesor,         a.nombre_rol = 'GESTOR CLIENTE_B',         a.es_activo = s.asesor_es_activo,         a.fecha_actualizacion = @AhoraUtc     FROM analitica.dim_asesor a     INNER JOIN #Asesores s         ON a.clave_cliente = @ClaveCliente        AND a.id_asesor_origen = CONVERT(VARCHAR(50), s.id_asesor_origen);      INSERT INTO analitica.dim_asesor     (         clave_cliente,         id_asesor_origen,         documento_asesor,         nombre_asesor,         nombre_rol,         es_activo     )     SELECT         @ClaveCliente,         CONVERT(VARCHAR(50), s.id_asesor_origen),         s.documento_asesor,         s.nombre_asesor,         'GESTOR CLIENTE_B',         s.asesor_es_activo     FROM #Asesores s     WHERE NOT EXISTS     (         SELECT 1         FROM analitica.dim_asesor a         WHERE a.clave_cliente = @ClaveCliente           AND a.id_asesor_origen = CONVERT(VARCHAR(50), s.id_asesor_origen)     );      IF OBJECT_ID('tempdb..#AdvisorMap') IS NOT NULL DROP TABLE #AdvisorMap;      SELECT         TRY_CONVERT(INT, a.id_asesor_origen) AS id_asesor_origen,         a.clave_asesor     INTO #AdvisorMap     FROM analitica.dim_asesor a     WHERE a.clave_cliente = @ClaveCliente       AND TRY_CONVERT(INT, a.id_asesor_origen) IN           (SELECT id_asesor_origen FROM #Asesores);       /* ------------------------------------------------------------------------        Supervisores observados/current.        ------------------------------------------------------------------------ */     IF OBJECT_ID('tempdb..#SupervisorSource') IS NOT NULL DROP TABLE #SupervisorSource;      SELECT DISTINCT         s.id_supervisor_origen,         NULLIF(LTRIM(RTRIM(su.cUsr_NroDoc)),'') AS documento_supervisor,         NULLIF         (             LTRIM(RTRIM(CONCAT(                 ISNULL(su.cUsr_Nombres,''),                 CASE WHEN NULLIF(LTRIM(RTRIM(su.cUsr_ApePat)),'') IS NOT NULL THEN ' ' ELSE '' END,                 ISNULL(su.cUsr_ApePat,''),                 CASE WHEN NULLIF(LTRIM(RTRIM(su.cUsr_ApeMat)),'') IS NOT NULL THEN ' ' ELSE '' END,                 ISNULL(su.cUsr_ApeMat,'')             ))),             ''         ) AS nombre_supervisor,         CONVERT(BIT, ISNULL(su.bEstado,0)) AS es_activo     INTO #SupervisorSource     FROM #Source s     INNER JOIN dbo.Crm_Usuario su         ON su.nId_Usuario = s.id_supervisor_origen     WHERE s.id_supervisor_origen IS NOT NULL;      /* current que no haya aparecido aún como líder en una operación MTD */     INSERT INTO #SupervisorSource     (         id_supervisor_origen,         documento_supervisor,         nombre_supervisor,         es_activo     )     SELECT DISTINCT         su.nId_Usuario,         NULLIF(LTRIM(RTRIM(su.cUsr_NroDoc)),''),         NULLIF         (             LTRIM(RTRIM(CONCAT(                 ISNULL(su.cUsr_Nombres,''),                 CASE WHEN NULLIF(LTRIM(RTRIM(su.cUsr_ApePat)),'') IS NOT NULL THEN ' ' ELSE '' END,                 ISNULL(su.cUsr_ApePat,''),                 CASE WHEN NULLIF(LTRIM(RTRIM(su.cUsr_ApeMat)),'') IS NOT NULL THEN ' ' ELSE '' END,                 ISNULL(su.cUsr_ApeMat,'')             ))),             ''         ),         CONVERT(BIT, ISNULL(su.bEstado,0))     FROM #Asesores x     INNER JOIN dbo.Crm_Usuario u         ON u.nId_Usuario = x.id_asesor_origen     INNER JOIN dbo.Crm_Usuario su         ON su.nId_Usuario = u.nid_UsuSuper     WHERE u.nid_UsuSuper IS NOT NULL       AND NOT EXISTS       (           SELECT 1           FROM #SupervisorSource q           WHERE q.id_supervisor_origen = su.nId_Usuario       );      UPDATE d     SET         d.documento_supervisor = s.documento_supervisor,         d.nombre_supervisor = COALESCE(s.nombre_supervisor, d.nombre_supervisor),         d.es_activo = s.es_activo,         d.fecha_actualizacion = @AhoraUtc     FROM analitica.dim_supervisor d     INNER JOIN #SupervisorSource s         ON d.clave_cliente = @ClaveCliente        AND d.id_supervisor_origen =            CONVERT(VARCHAR(50), s.id_supervisor_origen);      INSERT INTO analitica.dim_supervisor     (         clave_cliente,         id_supervisor_origen,         documento_supervisor,         nombre_supervisor,         es_activo     )     SELECT         @ClaveCliente,         CONVERT(VARCHAR(50), s.id_supervisor_origen),         s.documento_supervisor,         COALESCE(s.nombre_supervisor, CONCAT('Supervisor ', s.id_supervisor_origen)),         s.es_activo     FROM #SupervisorSource s     WHERE NOT EXISTS     (         SELECT 1         FROM analitica.dim_supervisor d         WHERE d.clave_cliente = @ClaveCliente           AND d.id_supervisor_origen =               CONVERT(VARCHAR(50), s.id_supervisor_origen)     );       /* ------------------------------------------------------------------------        Contacto detallado asesor/deudor/día.        ------------------------------------------------------------------------ */     IF OBJECT_ID('tempdb..#ContactoDetalle') IS NOT NULL DROP TABLE #ContactoDetalle;      SELECT         s.clave_fecha,         s.clave_cartera,         a.clave_asesor,         s.id_deudor_origen,         CONVERT(BIT, MAX(CASE WHEN s.codigo_contacto = 'CD' THEN 1 ELSE 0 END))             AS indicador_contacto_directo,         CONVERT(BIT, MAX(CASE WHEN s.codigo_contacto = 'CI' THEN 1 ELSE 0 END))             AS indicador_contacto_indirecto,         CONVERT(BIT, MAX(CASE WHEN s.codigo_contacto = 'NC' THEN 1 ELSE 0 END))             AS indicador_sin_contacto     INTO #ContactoDetalle     FROM #Source s     INNER JOIN #AdvisorMap a         ON a.id_asesor_origen = s.id_asesor_origen     GROUP BY         s.clave_fecha,         s.clave_cartera,         a.clave_asesor,         s.id_deudor_origen;       /* ------------------------------------------------------------------------        Promesa diaria por asesor, usando el hecho_promesa ya validado.        ------------------------------------------------------------------------ */     IF OBJECT_ID('tempdb..#PromesaDiaria') IS NOT NULL DROP TABLE #PromesaDiaria;      SELECT         s.clave_fecha,         s.clave_cartera,         a.clave_asesor,         COUNT_BIG(*) AS cantidad_promesas,         CONVERT(DECIMAL(19,4), SUM(ISNULL(p.monto_promesa,0))) AS monto_promesas     INTO #PromesaDiaria     FROM analitica.hecho_promesa p     INNER JOIN #Source s         ON s.id_operacion_origen = p.id_operacion_origen     INNER JOIN #AdvisorMap a         ON a.id_asesor_origen = s.id_asesor_origen     WHERE p.clave_cliente = @ClaveCliente       AND p.clave_campana = @ClaveCampana       AND p.es_promesa_valida = 1     GROUP BY         s.clave_fecha,         s.clave_cartera,         a.clave_asesor;       /* ------------------------------------------------------------------------
    ...
    (
        SELECT TOP (1)
            o.nId_UsuOpe
        FROM dbo.Crm_DocxCobrarOpe o
        INNER JOIN dbo.Crm_Usuario u
    ...
    LEFT JOIN integracion_datos_analitica.tipo_cambio_fallback_cliente_b fb
