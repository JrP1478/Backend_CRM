...
internal sealed class CrmGrupoClienteRepository(CrmDbContext context)
    : ICrmGrupoClienteRepository
{
    public async Task<IReadOnlyList<CrmGrupoCliente>> ObtenerTodosGruposActivosAsync(
        CancellationToken cancellationToken) =>
        await context.Crm_Grupos
            .AsNoTracking()
            .Where(group =>
                group.bEstado == true &&
                group.nid_cliente.HasValue &&
                group.nid_cliente.Value > 0)
            .Select(group => new CrmGrupoCliente
            {
    ...
    public async Task<IReadOnlyList<CrmGrupoCliente>> ObtenerGruposActivosAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return Array.Empty<CrmGrupoCliente>();
        }

        return await context.Crm_Grupos
            .AsNoTracking()
            .Where(group =>
                group.bEstado == true &&
                group.nid_cliente.HasValue &&
