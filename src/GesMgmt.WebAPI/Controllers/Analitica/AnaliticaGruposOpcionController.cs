...
[Route("v1/Analitica/Acceso/Opciones/{idOpcion:int}/Grupos")]
public sealed class AnaliticaGruposOpcionController(
    IContextoUsuarioAnalitica userContext,
    IAutorizacionAnaliticaService authorizationService,
    IAdministracionGruposOpcionAnaliticaService service) : AnaliticaControllerBase
{
    [HttpGet]
    [ProducesResponseType(typeof(AnaliticaGruposOpcionResponse), StatusCodes.Status200OK)]
    ...
    public async Task<IActionResult> ActualizarAsync(
        int idOpcion,
        [FromBody] ActualizarAnaliticaGruposOpcionRequest? request,
        CancellationToken cancellationToken)
    {
        var administrator = await RequerirAdministradorAsync(
            userContext,
